...
const statusColors = {
  'New Leads': 'bg-gray-400 text-gray-800 border-gray-200',
  'Qualified': 'bg-blue-400 text-blue-800 border-blue-200',
  'Proposal Sent': 'bg-amber-400 text-amber-800 border-amber-200',
  'Negotiation': 'bg-red-400 text-red-800 border-red-200',
  'Closed': 'bg-green-400 text-green-800 border-green-200'
};
...
export function LeadKanbanBoard() {
  ...
  return (
    <div className="space-y-6 lg:space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 neo-card p-4 border border-gray-300">
        <div>
      ...
      <div className="hidden lg:block">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-16 overflow-x-auto">
          {statusColumns.map((status) => {
            const statusLeads = getLeadsByStatus(status);
            const statusValue = statusLeads.reduce((sum, lead) => {
              const value = parseFloat(lead.value.replace(/[^\d.]/g, ''));
